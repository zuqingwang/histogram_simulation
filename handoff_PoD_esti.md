...
> 最后更新：2026-08-10（**v20 附1**：按用户澄清，模块 12 阈值曲线改为「点用直线相连的折线」
> 而非 `step` 阶梯；模块 11 图 ③ peak std 叠加 Gumbel 极值解析对照。前情：v20 在 v11 上追加
> 模块 11–17，补齐 4 条要求，三份全量扫描缓存跑完，无头验证 0 错误）。
...
| 12 | 40/41 | 阈值 vs bg **折线**（采样点用直线相连，非 `step` 阶梯）+ 连续（实数）阈值 $T_c$ | 1「不要阶梯」 |
