# -*- coding: utf-8 -*-
"""
build_crosstalk_v13.py —— 生成 crosstalk_sim_v13.ipynb
========================================================
...
v13 相对 v12（本版；仅可视化/呈现，核心检测算法不动）：
  A) cell5 只保留 kick 栅格图（删掉时间轴放大下图），比例改约 4:3 更紧凑；
     格内文字改为编码 "tx + delta/12"（如 50+3/12），delta 取 Excel tdelay 原值仅展示。
  B) cell9 加开关 PLOT_NEGLIGIBLE（默认 False）：是否画出空间可忽略鬼影；柱内文字改为 "LxKx"。
  C) 删除所有 plt.savefig 保存图片操作（只在 notebook 内展示，不落地 PNG）。
...
CELL_HEADER = r'''# 串扰仿真 v13 —— 长焦 A 组 16 激光器编码鬼影检测（理想 δ 回波）
...
cells = [
    md_cell("v13_header", CELL_HEADER),
    code_cell("v13_imports", CELL_IMPORTS),
    code_cell("v13_excel", CELL_EXCEL),
    code_cell("v13_encoding", CELL_ENCODING),
    code_cell("v13_timing_plot", CELL_TIMING_PLOT),
    code_cell("v13_detect", CELL_DETECT),
    code_cell("v13_verify", CELL_VERIFY),
    code_cell("v13_echo_map", CELL_ECHO_MAP),
    code_cell("v13_cumulative", CELL_CUMULATIVE),
    code_cell("v13_summary", CELL_SUMMARY),
]
