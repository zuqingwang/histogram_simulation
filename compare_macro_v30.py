...
CACHE = "compare_macro_v30_cache.npz"
CACHE_CKPT = "compare_macro_v30_cache.partial.npz"
...
def main():
    ...
    ap.add_argument("--n-mc", type=int, default=600_000)   # ★ v30：由 300k 提到 600k
...
def report(res, amb, n_mc):
    ...
    def curves_vs_bg(a, yfun, ylab, title):
        for cfg in CONFIGS:
            ...
            a.plot(bg[ci][msk], yfun(ci, msk), CFG_LS[cfg[0]], color=CFG_COLOR[cfg],
                   marker="o", ms=2.4, lw=1.7, label=label(cfg))
        # ★ v30：改线性横轴（对数轴把低 bg 段拉得过宽，看不清工作区）
    ...
    for (ca, cb), col in zip(pairs, ["tab:red", "tab:orange", "tab:green", "tab:purple"]):
        ...
        if not (hi > lo):
            continue
        xs = np.linspace(lo, hi, 120)      # ★ v30：线性公共网格
