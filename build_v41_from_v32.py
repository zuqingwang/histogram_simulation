...
OUT_NB = "lidar_histogram_sim_v41.ipynb"

# ============================================================================
# 停用集（转 markdown 注释停用）——相对 v40 变化：
#   恢复 23/33/43/47（从停用集移除）；cell25 特殊拆分处理（不在此集）；
#   保持停用 29/41/53/55。
# ============================================================================
DISABLE = {
    29: "模块 9：阵列内不同 SPAD 的响应差异 —— v41 不需要，停用。",
    41: "模块 17：不同反射率 ρ 的信号波形对比 —— 被『模块 19 能量扫描』取代，停用。",
    53: "模块 15：多次蒙卡的 SNR 分布 + 正态拟合 —— v41 用『模块 20 SNR-距离』替代，停用。",
    55: "模块 16：100ppm 噪点率理论阈值 + 海量蒙卡验证 —— v41 不需要，停用。",
}
...
def to_markdown_disabled(cell, reason, ver="v41"):
...
DEX_STEP  = 0.05          # 对数步长 [dex]（每档 boost 乘 10^0.05≈1.122；范围跨 4 dex → 约 81 档）
K_TH_19   = 5.0           # 检测阈值倍数 det_th = K_TH_19 · nc_base（沿用 v32 模块14 默认）
COG_HALF  = 6            # 重心法窗口半宽 [bin]

# 对数等分：从 log10(BOOST_MIN) 到 log10(BOOST_MAX)，步长 DEX_STEP dex
boost_grid = 10.0 ** np.arange(np.log10(BOOST_MIN), np.log10(BOOST_MAX) + DEX_STEP/2, DEX_STEP)
...
print(f"模块19 能量扫描：boost ∈ [{BOOST_MIN:.0e}, {BOOST_MAX:.0e}], 对数步长 {DEX_STEP}dex → {N_BOOST} 档（逐光子引擎）")
...
# ---- 绘图：dist-peak / dist-area 四条曲线（横轴 linear）----
...
plt.suptitle(f"模块 19 — 能量扫描定时 (cali_dist={D0} m, N_shots={N_shots}, K_th={K_TH_19:.0f}, 逐光子二值 MC)",
             fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig("v41_energy_scan.png", dpi=110, bbox_inches="tight")
plt.show()
...
plt.suptitle(f"模块 20 — SNR & 信号率 vs 距离：与平方反比 1/D² 对比 (ρ={echo0['rho']}, N_shots={N_shots}, {N_D_B}档)",
             fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig("v41_snr_vs_distance.png", dpi=110, bbox_inches="tight")
plt.show()
...
# 追加 v41 新模块
out_cells.append(md_cell("v41_new_header", NEW_HEADER_MD))
out_cells.append(code_cell("v41_module_19_energy", MODULE_19))
out_cells.append(code_cell("v41_module_20_snr", MODULE_20))
