...
#   它对应一个矩形接收视场（角尺寸 = `B·pitch/f_RX`，探测面窗口 = `(Bx·pitch)×(By·pitch)`）。
#   **只有落在这个窗口内的回波像斑光才被接收**，超出窗口的像斑光丢失。
#   对比 **6×3 / 9×3 / 12×3** 三种宏像元尺寸（x=短边取 6/9/12、y=长边固定 3）对直方图的影响。
#
# **v2 已有的修改**
...
#
# **为什么像斑比原始发光区小约一个数量级？** 因为 `x_D ∝ D`（发散）而放大率 `m=f_RX/D ∝ 1/D`，
# 两者相乘 **D 抵消**，故远场像斑 = `发光区 × f_RX/f_TX`（与距离无关）。本例缩小倍率就是
# `f_TX/f_RX = 236/25 ≈ 9.4×`：60µm→6.4µm(⊕艾里→~7.6µm)、1150µm→~122µm。这是长焦发射+短焦接收的必然结果。
#
...
# 回波像斑是**椭圆高斯**（1/e² 全宽 `sx×sy`，来自模块5；x=短边≈7µm、y=长边≈122µm），高斯标准差 `σ=s/4`
# （因 `exp(-2r²/(s/2)²)` 的 1/e² 半宽为 s/2 ⇒ σ=s/4）。矩形窗口 `[-W/2, W/2]` 居中对准像斑中心时，
# 一维高斯在窗口内的积分为误差函数（erf），二维为其积（σ_x 配短边 x、σ_y 配长边 y）：
