# -*- coding: utf-8 -*-
"""PoD_esti v30 模块 8 / 14 的信号扫描（固定信号强度 × 统一 bg 网格），多进程 + 缓存。
...
产出缓存 `pod_esti_v30_cache_signal.npz`，字段与 notebook 的 CACHE_SIG 完全兼容：
...
用法：
    ...
    python run_pod_v30_sig_scan.py --workers 20
    python run_pod_v30_sig_scan.py --limit 3 --n-mc 500     # 冒烟
"""
...
# ★ v30：全量重算，不再回落到旧版内核，避免混用不同 FAR 列表的缓存
import pod_esti_v30_core as core

CACHE = "pod_esti_v30_cache_signal.npz"
CACHE_CKPT = "pod_esti_v30_cache_signal.partial.npz"
...
def main():
    ...
    print("=" * 84)
    print(f"v30 信号扫描：{ng} bg × {nb} boost × N={N_LIST} = {ng*nb*len(N_LIST)} 档，"
          f"每档 {args.n_mc:,} MC")
