# -*- coding: utf-8 -*-
"""从 PoD_esti_v10.ipynb 提取计算内核 → pod_esti_v10_core.py（供多进程 import）。
...
NB = Path("PoD_esti_v10.ipynb")
OUT = Path("pod_esti_v10_core.py")
CALC_CELLS = [2, 4, 6, 8, 9, 17, 22, 25]
CELL17_CUT = "# ---- ★ v10：按单次 noise 网格扫一次"
...
def main():
    ...
    for idx in CALC_CELLS:
        ...
        if idx == 25:
            ...
            src = src[:cut].rstrip() + "\n"
        # 去掉 cell 22 里依赖 NOISE_RES 的 THRESH 自动构建（core import 时 NOISE_RES 可能为空）
        if idx == 22:
            ti = src.find("\nTHRESH = {}")
            if ti >= 0:
                src = src[:ti].rstrip() + "\n"
        parts.append(f"\n# ===== 源自 PoD_esti_v10.ipynb cell {idx} =====\n")
        parts.append(src)
    parts.append(FOOTER)
