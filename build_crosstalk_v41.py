# -*- coding: utf-8 -*-
"""
build_crosstalk_v41.py —— 生成 crosstalk_sim_v41.ipynb
========================================================
...
OUT_NB = "crosstalk_sim_v41.ipynb"
# v40：离散字母表（预算 100ns）。
...
OVERVIEW = f"""# 串扰仿真 v41 —— 可变 kick 间隔时序 + 离散 tcode + 四步滤噪

**基于 v40 参数**（字母表、XM、对射、四步流程不变），时序模型升级如下。

## v41 时序（相对 v40）

v40：$t_{{\\mathrm{{fire}}}} = k \\cdot 2.2\\,\\mu s + \\mathrm{{tcode}} + \\delta$（kick 等间隔）

v41：**kick 仅作触发**，编码在每次发光之前；**kick 间隔不固定**：

1. $T_{{\\mathrm{{kick}}}}[k]$ — 第 $k$ 次 kick 触发时刻  
2. $\\mathrm{{enc}}[l,k] = d_{{\\mathrm{{FPGA}}}}[k] + \\mathrm{{tcode}}[l,k] + \\delta[k]/12$（ns 步进）  
3. $t_{{\\mathrm{{fire}}}}[l,k] = T_{{\\mathrm{{kick}}}}[k] + \\mathrm{{enc}}[l,k]$  
4. 固定等待 $T_{{\\mathrm{{fixed}}}} = 2.2\\,\\mu s$（TOF 段）  
5. $T_{{\\mathrm{{kick}}}}[k+1] = \\max_l t_{{\\mathrm{{fire}}}}[l,k] + T_{{\\mathrm{{fixed}}}}$

因此 $\\Delta T_k = T_{{\\mathrm{{kick}}}}[k+1]-T_{{\\mathrm{{kick}}}}[k] = \\max_l\\mathrm{{enc}}[l,k] + 2.2\\,\\mu s$（随编码变化）。

**对射雷达**采用相同编码表与相同时序结构；独立 FPGA 抖动序列（步骤 iii）。

**扫角总时长**：整段 kick 链总用时需满足 `TIMING.scan_budget_us`（可在全局参数中改）。

## v40 继承：离散字母表

相对 v30：**tcode 不再用连续整数 `0..B`**，而是从有限字母表里取值。
...
- 全局参数在 **第一个代码 cell**（改参数只动那里）
- `TIMING.tof_fixed_us = 2.2`（固定 TOF 段）；编码段含 tcode + 可选 FPGA
...
TIMING = {{
    "tof_fixed_us": 2.2,            # 固定 TOF 等待段 [μs]（kick 间隔的恒定部分）
    "tof_window_ns": 2000.0,        # 单次发光后直方图记录窗 [ns]
    "use_tx_trig_dly": True,        # 编码含 tcode / Excel tx_trig_dly（1 ns 步长）
    "use_delta_dly": False,         # 编码含 delta_dly/12（1/12 ns 步长，按 kick）
    "fpga_in_encode": True,         # 步骤 iii：FPGA 累计抖动计入 enc（kick 后、发光前）
    "scan_budget_us": 35.2,         # 整角度扫完时限 [μs]；None=不检查
...
RADAR = {{
    ...
    "phase_ns": 700.0,
    "same_model": True,
    "same_timing": True,            # 对射采用相同 kick 链 + 编码表
    "same_tcode": True,
}}
...
TOF_WINDOW = TIMING["tof_window_ns"] * NS
...
print("0. 全局参数已加载")
print(f"  TOF_FIXED={{TIMING['tof_fixed_us']}}μs  TOF_WINDOW={{TIMING['tof_window_ns']:.0f}}ns  "
      f"D_UNAMBIG={{D_UNAMBIG:.1f}}m  角度={{TIMING['scan_angle_deg']}}°")
if TIMING.get("scan_budget_us"):
    print(f"  扫角预算 scan_budget_us={{TIMING['scan_budget_us']}}μs")
...
print(f"2. 从 {EXCEL_FILE} [长焦/A组] 读取：{N_LASERS} 激光器，"
...
plot_kick_grid(lambda lid, k: code_label(EXCEL_TX[(lid, k)], k),
               f"图 V41-1  Excel 编码栅格（{N_LASERS}×16，"
...
plt.suptitle(f"图 V41-2  离散 tcode（预算 {B25}ns）", fontsize=14)
plt.tight_layout(); plt.show()
...
def encode_delay_ns(lid, kick, use_tcode=False, cum_ns=None):
    ...
    return d_fpga + t_tx + d_off


def build_kick_schedule(use_tcode=False, cum_encode_ns=None):
    """
    v41 时序链：
      T_kick[k] → +enc → t_fire → +TOF_FIXED → T_kick[k+1]
    返回 (firings, schedule)。
    """
    all_kicks = sorted(set(k for lid in laser_ids for k in SHOT_KICKS[lid]))
    T_kick = 0.0
    kick_times, kick_period_ns, firings = [], [], []
    for ki, k in enumerate(all_kicks):
        kick_times.append(T_kick)
        t_fires_k = []
        for lid in laser_ids:
            if k not in SHOT_KICKS[lid]:
                continue
            enc_ns = encode_delay_ns(lid, k, use_tcode, cum_encode_ns)
            ...
            t_fires_k.append(t_fire)
        if t_fires_k:
            T_next = max(t_fires_k) + TOF_FIXED
            if ki + 1 < len(all_kicks):
                kick_period_ns.append((T_next - T_kick) / NS)
            T_kick = T_next
    total_end = max((f["t_fire"] for f in firings), default=0.0) + TOF_FIXED
    schedule = {
        "kick_times_s": kick_times,
        "kick_period_ns": kick_period_ns,
        "kick_period_map": {all_kicks[i]: kick_period_ns[i]
                            for i in range(len(kick_period_ns))},
...
def build_firings(use_tcode=False, cum_encode_ns=None):
    firings, _ = build_kick_schedule(use_tcode, cum_encode_ns)
    return firings
...
def inject_radar(recs, D, schedule, cum_ours=None, cum_radar=None, phase_ns=None):
    ...
    period_map = schedule["kick_period_map"]
    ...
    for lid in laser_ids:
        for k in SHOT_KICKS[lid]:
            ...
            period = period_map.get(k, fallback)
...
def run_scene(use_tcode=False, use_fpga=False, D=None, phase_ns=None,
    ...
    firings, schedule = build_kick_schedule(use_tcode=use_tcode,
                                            cum_encode_ns=cum_enc)
...
def plot_timing_timeline(firings, schedule, title, cum_radar=None, show_radar=True):
    """某一角度的 kick→编码→发光→TOF 时序图（μs）；对射雷达同编码结构。"""
    fig, ax = plt.subplots(figsize=(15, 5.5))
    ymap = {lid: i for i, lid in enumerate(laser_ids)}
    enc_h, fire_h, tof_h = 0.22, 0.28, 0.18
    for f in firings:
        lid, k = f["laser"], f["kick"]
        y = ymap[lid]
        t0 = f["t_kick"] * 1e6
        enc_us = f["encode_ns"] * 1e-3
        t_fire = f["t_fire"] * 1e6
        ax.barh(y, enc_us, left=t0, height=enc_h, color="#aed6f1",
                edgecolor="#1a5276", linewidth=0.5, label="编码" if k == 0 and lid == laser_ids[0] else "")
        ax.barh(y + 0.32, t_fire - t0, left=t0, height=fire_h, color=laser_color(lid),
                edgecolor="k", linewidth=0.4, alpha=0.85)
        ax.barh(y + 0.32, TIMING["tof_fixed_us"], left=t_fire, height=tof_h,
                color="#d5dbdb", edgecolor="gray", linewidth=0.4,
                label="TOF固定段" if k == 0 and lid == laser_ids[0] else "")
        ax.plot([t0, t0], [y - 0.1, y + 0.65], "k--", lw=0.7, alpha=0.5)
        ax.text(t0, y - 0.15, f"K{k}", fontsize=6, ha="center")
    # kick 间隔标注
    for i, k in enumerate(schedule["all_kicks"][:-1]):
        if i < len(schedule["kick_period_ns"]):
            dt = schedule["kick_period_ns"][i] * 1e-3
            t_k = schedule["kick_times_s"][i + 1] * 1e6
            ax.annotate(f"Δ={dt:.2f}μs", xy=(t_k, len(laser_ids) - 0.3),
                        fontsize=7, color="#566573", ha="center")
    if show_radar and RADAR["enable"] and cum_radar is not None:
        ax.axhline(len(laser_ids) + 0.5, color=RADAR_COLOR, ls=":", lw=1.2, alpha=0.6)
        ax.text(0.02, len(laser_ids) + 0.55, "对射雷达：同编码表/同时序链，独立 FPGA",
                transform=ax.get_yaxis_transform(), fontsize=8, color=RADAR_COLOR)
    ax.set_yticks([ymap[l] + 0.32 for l in laser_ids])
    ax.set_yticklabels([f"L{l}" for l in laser_ids])
    ax.set_xlabel("时间 [μs]")
    ax.set_title(title + f"  总时长={schedule['total_scan_us']:.2f}μs"
                 + (f"  角度={TIMING['scan_angle_deg']}°" if TIMING.get("scan_angle_deg") is not None else ""))
    bud = TIMING.get("scan_budget_us")
    if bud is not None:
        ok = schedule.get("within_budget", schedule["total_scan_us"] <= bud)
        ax.axvline(bud, color="#e74c3c" if not ok else "#27ae60", ls="--", lw=1.0,
                   label=f"扫角预算 {bud}μs")
    ax.legend(loc="upper right", fontsize=8)
    ax.grid(alpha=0.2, axis="x")
    plt.tight_layout(); plt.show()
    print(f"  kick 周期 [μs]: " +
          ", ".join(f"K{schedule['all_kicks'][i]}→{schedule['kick_period_ns'][i]*1e-3:.3f}"
                    for i in range(min(8, len(schedule['kick_period_ns'])))))
    if bud is not None:
        print(f"  扫角总时长 {schedule['total_scan_us']:.2f}μs / 预算 {bud}μs  "
              f"{'✓' if schedule.get('within_budget') else '✗'}")
...
print("4. 核心函数已加载（v41 可变 kick 间隔）")
...
# 3c. 时序链演示（当前角度）
# ============================================================================
_f0, _sch0 = build_kick_schedule(use_tcode=False)
plot_timing_timeline(
    _f0, _sch0,
    f"图 V41-0  Excel 编码时序链（角度 {TIMING['scan_angle_deg']}°，"
    f"kick 间隔 = max(enc)+{TIMING['tof_fixed_us']}μs）")
_f1, _sch1 = build_kick_schedule(use_tcode=True)
plot_timing_timeline(
    _f1, _sch1,
    f"图 V41-0b  +tcode 后时序链（间隔随编码变化）")
plot_kick_grid(
    lambda lid, k: f"{encode_delay_ns(lid, k, use_tcode=True):.0f}",
    f"图 V41-0c  格内 = enc [ns]（FPGA+tcode+delta，tcode 模式）")
'''
...
plot_echo_channel(S_i["hs"], S_i["rs"], SCENE["demo_laser"],
                  f"图 V41-i-a  L{SCENE['demo_laser']} Excel 基线（滤后堆叠）",
                  S_i["D"])
plot_echo_grid(S_i["hs"], S_i["rs"],
               f"图 V41-i-b  16 宫格：Excel 基线，鬼影与对射都应留下",
               S_i["D"])
'''
...
plot_timing_timeline(
    ...
    f"图 V41-iii-0  双方 FPGA 编码时序（角度 {TIMING['scan_angle_deg']}°）",
...
axes[0].set_title(f"图 V41-iii-a  双方 FPGA 累计延时"
...
axes[1].set_title("图 V41-iii-b  对射相对我方的净抖动（决定直方图落点）")
...
plot_true_radar_stack(
    ...
    f"图 V41-iii-c  L{lid}：双方 jitter 后真回波 vs 对射（堆叠柱）",
    S_iii["D"])

plot_echo_channel(S_iii["hs"], S_iii["rs"], lid,
                  f"图 V41-iii-d  L{lid} 双方FPGA后：对射应被打散并被XM丢掉",
                  S_iii["D"])
plot_echo_grid(S_iii["hs"], S_iii["rs"],
               "图 V41-iii-e  16 宫格：tcode+双方FPGA 后鬼影与对射都应↓",
               S_iii["D"])
'''
...
for ax in axes:
    ax.set_ylabel("堆叠层数")
fig.legend(handles=type_legend(), loc="upper center", ncol=4, fontsize=9)
fig.suptitle(f"图 V41-iv-a  L{lid} 三角度 = 三次独立 iii 仿真"
...
for q in line_iv[lid]["peaks"]:
    ...
    if any(r.get("is_true") for r in recs):
        n_true_line_drop += 1
fig.legend(handles=type_legend(), loc="upper center", ncol=4, fontsize=9)
fig.suptitle(f"图 V41-iv-b  L{lid} 一字滤波前后"
...
plt.suptitle("图 V41-对比  i→ii→iii：tcode 清鬼影，双方FPGA 清对射", fontsize=13)
plt.tight_layout(); plt.show()
'''
...
def main():
    cells = [
        md_cell("v41_overview", OVERVIEW),
        code_cell("v41_params", CELL_PARAMS),
        code_cell("v41_import", CELL_IMPORT),
        code_cell("v41_excel", CELL_EXCEL),
        code_cell("v41_grid", CELL_GRID),
        code_cell("v41_tcode", CELL_TCODE_LOAD),
        code_cell("v41_core", CELL_CORE),
        code_cell("v41_line", CELL_LINE),
        md_cell("v41_s0_doc",
                "# 3c. 时序链演示\n\n"
                "kick 只触发；**编码在 kick 后、发光前**；发光后等固定 $2.2\\,\\mu s$ TOF 段；"
                "下一 kick 间隔 $=\\max\\mathrm{enc}+2.2\\,\\mu s$（随 tcode 变化）。\n"
                "对射雷达采用相同编码结构。"),
        code_cell("v41_s0", CELL_S0),
        md_cell("v41_si_doc", "# 4.i  Excel 时序基线\n\n不加 tcode / FPGA，只看鬼影与对射。"),
        code_cell("v41_si", CELL_S1),
        md_cell("v41_sii_doc", "# 4.ii  加入 tcode\n\n模组内鬼影应被打散并被 XM 丢掉；对射仍在。"),
        code_cell("v41_sii", CELL_S2),
        md_cell("v41_siii_doc",
                "# 4.iii  加入双方 FPGA 累计抖动\n\n"
                "最坏对射：同型号、同时序、同 tcode；**我方与对射雷达都有 FPGA**。\n"
                "FPGA 抖动计入编码段；对射相对落点 "
                "$$(\\phi + d_{\\mathrm{radar}} - d_{\\mathrm{ours}})\\bmod T_{\\mathrm{kick}}[k]$$"
                "（每 kick 周期可变）。"),
        code_cell("v41_siii", CELL_S3),
        md_cell("v41_siv_doc",
                "# 4.iv  加入一字滤波\n\n"
        ...
        code_cell("v41_siv", CELL_S4),
        code_cell("v41_compare", CELL_COMPARE),
        code_cell("v41_summary", CELL_SUMMARY),
    ]
    nb = {
        "cells": cells,
        "metadata": {
