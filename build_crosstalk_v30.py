# -*- coding: utf-8 -*-
"""
build_crosstalk_v30.py —— 生成 crosstalk_sim_v30.ipynb
========================================================
...
OUT_NB = "crosstalk_sim_v30.ipynb"
# 码表验收标准：1~600m 细扫（步长 2m）鬼影残留 = 0 且真峰误杀 = 0，
# 在此前提下取最短预算。约束（散开+避真）硬下界为 24ns。
TBL_R15 = os.path.join("docs", "tcode", "tcode_table_zero_r1.5_32ns.py")
...
OVERVIEW = f"""# 串扰仿真 v30 —— 参数字典驱动的四步滤噪
...
- tcode 码表验收标准：**必须完全滤干净**——1~600m 扫描鬼影残留 0、真峰误杀 0；
  满足后才在零残留解里压最短预算。
  当前 `XM.ratio=1.5` → **{B15}ns**；`XM.ratio=2.5` → **{B25}ns**（= 约束硬下界）
...
TCODE = {{
    "enable": True,
    "ratio_mode": "1.5",        # 跟 XM.ratio 对齐："1.5" → {B15}ns；"2.5" → {B25}ns
    "consider_gap": True,       # v30 默认：考虑激光不串扰距离
    "max_gap": 2,               # 编号间隔 > max_gap 视为可忽略
    "sep_ns": {SEP15},
    "budget_ns_r15": {B15},     # ratio=1.5 零残留最短预算
    "budget_ns_r25": {B25},     # ratio=2.5 零残留最短预算（= 约束硬下界）
}}
...
#    并在满足该条件的解里压到最短预算：
#      ratio=1.5 → {B15}ns    ratio=2.5 → {B25}ns（= 约束硬下界）
# ============================================================================
TCODE_R15 = {{
{table_literal(T15)}
}}
...
def main():
    cells = [
        md_cell("v30_overview", OVERVIEW),
        code_cell("v30_import", CELL_IMPORT),
        code_cell("v30_excel", CELL_EXCEL),
        code_cell("v30_grid", CELL_GRID),
        code_cell("v30_params", CELL_PARAMS),
        code_cell("v30_tcode", CELL_TCODE_LOAD),
        code_cell("v30_core", CELL_CORE),
        code_cell("v30_line", CELL_LINE),
        md_cell("v30_si_doc", "# 4.i  Excel 时序基线\n\n不加 tcode / FPGA，只看鬼影与对射。"),
        code_cell("v30_si", CELL_S1),
        md_cell("v30_sii_doc", "# 4.ii  加入 tcode\n\n模组内鬼影应被打散并被 XM 丢掉；对射仍在。"),
        code_cell("v30_sii", CELL_S2),
        md_cell("v30_siii_doc",
                "# 4.iii  加入双方 FPGA 累计抖动\n\n"
                "最坏对射：同型号、同时序、同 tcode；**我方与对射雷达都有 FPGA**。\n"
                "相对落点 $$(\\phi + d_{\\mathrm{radar}} - d_{\\mathrm{ours}})\\bmod T_{\\mathrm{kick}}$$。"),
        code_cell("v30_siii", CELL_S3),
        md_cell("v30_siv_doc",
                "# 4.iv  加入一字滤波\n\n"
        ...
        code_cell("v30_siv", CELL_S4),
        code_cell("v30_compare", CELL_COMPARE),
        code_cell("v30_summary", CELL_SUMMARY),
    ]
    nb = {
        "cells": cells,
        "metadata": {
