# -*- coding: utf-8 -*-
"""PoD_esti v10 计算内核（自 v05 分叉；支持 per-shot hist_i + 前缀和）。

v10 约定：
  - 每次实现最多仿真 N_SHOTS_MAX=4 发，得到 hist_i（i=1..4）；
  - N∈{1,2,4} 的 hist_add = sum(hist_1..hist_N)（取前缀，避免重复仿真）；
  - noise = 单次 hist_i 统计窗均值；bg = hist_add 统计窗均值；peak 在 hist_add 上统计。
  - 本模块 import 时只构建物理量，不自动跑 MC（扫描见 run_pod_v10_scan.py）。
原 v05 头注：
...
N_SHOTS_LIST = [1, 2, 4]         # ★ v10：由前缀和派生，不各自独立扫

# ---- ★ v10：噪声网格按【单次 hist_i 的 noise】线性等间距 ----
#   所有 N 共用同一环境；bg≈N·noise（平衡态）。
NOISE_GRID_AMB = np.round(np.arange(0.25, 12.0 + 1e-9, 0.25), 4)  # 48 档
NOISE_GRID = {n: NOISE_GRID_AMB.copy() for n in N_SHOTS_LIST}  # 兼容旧键名
...
N_MC_NOISE  = 200_000         # v10：够 1% FAR；全量重算         # 每档纯噪声 MC 条数（10 ppm 需 ≥1e6 才有约 10 个越阈事件）
...
CACHE_SIG   = "pod_esti_v10_cache_signal.npz"
CACHE_POD   = "pod_esti_v10_cache_pod.npz"  # 预留；本版先不跑全量 PoD
CACHE_NOISE_FALLBACK = []  # ★ v10 禁止复用旧缓存
CACHE_POD_FALLBACK   = []
...
def noise_hists_per_shot(n_real, n_shots, r_det, rng, inv_tab=None):
    """纯噪声：返回 hist_i，形状 (n_real, n_shots, NBINS)，每 shot 计数 ∈[0,27]。"""
    if inv_tab is None:
        inv_tab = build_inv_table(r_det)
    out = np.zeros((n_real, n_shots, NBINS), dtype=np.int32)
    for s in range(n_shots):
        # 每发独立 RNG 前进；n_tr=27 = 单发宏像元
...
def binary_macro_stepping_per_shot(n_real, f_pix, n_shots, r_sig_unit, tgrid, r_amb,
    ...
    """信号+环境：返回 hist_i (n_real, n_shots, len(centers))。

    把 n_shots×27 条轨迹一次仿真，再按 shot 维对 27 个 SPAD 求和。
    """
    f_arr = np.tile(np.asarray(f_pix, float), int(n_shots))
    dt = tgrid[1] - tgrid[0]
    n_tr = f_arr.size
    n_pix = int(np.asarray(f_pix).size)
    assert n_tr == n_pix * n_shots
    nb = len(centers)
    k_max = int(np.ceil(20.0 * tau_rc / dt))
    phi = pde * spad_response_g(1.0 - np.exp(-np.arange(k_max + 1) * dt / tau_rc),
                                resp_shape, resp_k)
...
def hist_add_from_prefix(hist_i, n_shots):
    """hist_add = sum(hist_1..hist_N)，N≤hist_i.shape[1]。"""
    return hist_i[:, :n_shots, :].sum(axis=1)


def stats_from_hist_i(hist_i, n_shots_list=None, i0=None, i1=None):
    """由一次 N_SHOTS_MAX 仿真的 hist_i，派生各 N 的 noise/bg/peak 充分统计量。

    返回 dict[N] = {noise_sum, noise_sumsq, bg_sum, bg_sumsq, peak_cnt, n}
    - noise：单次 hist_i 在统计窗的均值（对前 N 发再平均，降低方差）
    - bg / peak：在 hist_add 的 [i0,i1) 窗上
    """
    ...
    out = {}
    # 各单发底噪
    shot_nz = hist_i[:, :, i0:i1].mean(axis=2)  # (n_real, n_max)
    for n in n_shots_list:
        if n > n_max:
            raise ValueError(f"n_shots={n} > hist_i 发数 {n_max}")
        hadd = hist_add_from_prefix(hist_i, n)
        a = hadd[:, i0:i1]
        bg = a.mean(axis=1)
        pk = a.max(axis=1)
        nz = shot_nz[:, :n].mean(axis=1)  # 用前 N 发的单发均值再平均
        n_tr = N_PIX_MACRO * n
        out[n] = {
            "n": n_real,
            "noise_sum": float(nz.sum()),
            "noise_sumsq": float((nz * nz).sum()),
            "bg_sum": float(bg.sum()),
            "bg_sumsq": float((bg * bg).sum()),
...
print("  · noise_hists_per_shot / binary_macro_stepping_per_shot —— ★v10 per-shot")
print("  · hist_add_from_prefix / stats_from_hist_i —— ★v10 前缀和统计")
...
# ---- ★ v10：import 时不跑 MC；扫描由 run_pod_v10_scan.py 负责 ----
NOISE_RES = {}
THRESH = {}
...
def _peaks_chunk(boost, n_shots, r_amb, n_real, seed):
    """单块 MC：始终仿 N_SHOTS_MAX 发，再取前 n_shots 前缀和的 peak。"""
...
def _signal_chunk_all_N(boost, r_amb, n_real, seed, n_shots_list=None):
    """一次仿真返回各 N 的信号窗 peak bincount（hist_add 上）。"""
    if n_shots_list is None:
        n_shots_list = N_SHOTS_LIST
    rng = np.random.default_rng(seed)
    hist_i = binary_macro_stepping_per_shot(
    ...
    out = {}
    for n in n_shots_list:
        pk = hist_add_from_prefix(hist_i, n).max(axis=1)
        n_tr = N_PIX_MACRO * n
        out[n] = np.bincount(pk, minlength=n_tr + 2).astype(np.int64)
    return out
