# -*- coding: utf-8 -*-
"""
build_crosstalk_v12.py —— 生成 crosstalk_sim_v12.ipynb
========================================================
...
v11 相对 v10：可视化增强（kick 栅格、鬼影来源标注/打印、逐回波溯源）。

v12 相对 v11（本版；仅可视化/统计，核心检测算法不动）：
  A) 全图统一配色 laser_color()：HSV 只变色相、明度/饱和度恒定，同一激光器恒定同色，
     解决 v11 用 tab20 相邻激光器"一深一浅"的问题。
  B) cell9 改为【逐回波堆叠柱】：每个回波一个带边框的柱、同记录距离处上下叠、柱内写发射 kick 号。
  C) 新增全局 CROSSTALK_MAX_GAP（默认 2）：激光器编号间隔 > 此值的串扰视为空间可忽略，
     各图/打印里弱化或标记；detect_echoes 仍忠实记录全部回波，只多打一个 negligible 标志。
...
CELL_HEADER = r'''# 串扰仿真 v12 —— 长焦 A 组 16 激光器编码鬼影检测（理想 δ 回波）

> **目标**：忠实记录"谁发光 → 碰到多远的物体 → 被谁在自己的 TOF 窗里收到 → 算成了多远"，
> 用几个特定距离验证仿真逻辑是否正确。暂不做全距离扫描、不做波形/蒙卡（那是 v44 的事）。
>
> **v12 相对 v11**：
> ① **全图统一配色**（`laser_color`，HSV 只变色相、明度饱和度恒定）——同一激光器在所有图里恒定同色，
>    解决 v11 用 tab20 导致相邻激光器"一深一浅"的问题；
> ② **cell9 逐回波堆叠柱**：每个回波画成一个带边框的柱、同一记录距离处上下叠加、柱内写发射 kick 号，
>    不同 kick/来源天然区分；
> ③ 新增全局 **`CROSSTALK_MAX_GAP`**：激光器编号间隔 > 该值的串扰视为空间上可忽略（默认 2，可改），
>    在各图/打印里弱化或标记。
...
ax = axes[0]
# 每个激光器在每个 kick 是否发光：画方块 + 标 tx。颜色用全局统一 laser_color()（同一激光器恒定同色）。
for e in firings:
...
ax.set_ylabel("激光器编号")
ax.set_title(f"发光 kick 栅格（{N_LASERS} 激光器 × 16 kick；格内数字 = tx_trig_dly[ns]，色=激光器·全图统一）")
...
for e in zoom_fires:
    ...
    ax.barh(e["laser"], width=3, left=off_ns, height=0.6,
            color=laser_color(e["laser"]), edgecolor="k", linewidth=0.5, alpha=0.95)
...
plt.tight_layout()
plt.savefig("crosstalk_v12_timing.png", dpi=110, bbox_inches="tight")
plt.show()
print("已保存 crosstalk_v12_timing.png")
'''
...
plt.savefig("crosstalk_v12_echo_map.png", dpi=110, bbox_inches="tight")
plt.show()
print("已保存 crosstalk_v12_echo_map.png")
...
CELL_CUMULATIVE = r'''# ============================================================================
# 绘图 3：每个激光器的【4 次发光累加 TOF 波形】—— v12 逐回波堆叠柱
# ============================================================================
# 需求(v12)：每一个回波（不同 kick 要区分开）都画成一个【带边框的柱】，同一记录距离处
#   上下叠加；柱内写"发射 kick 号"，颜色=发射激光器。这样能看清每条回波的来源。
...
for idx, lid in enumerate(laser_ids):
    ...
    for kbin, items in stacks.items():
        ...
        for level, x in enumerate(items):
            ...
            # 柱内写 kick 号（不同 kick 天然区分开）
            ax.text(xc, level + 0.45, f"K{x['emit_kick']}", fontsize=6.0,
    ...
    ax.set_title(f"L{lid}（自{n_self}·鬼(近){n_keep}·鬼(远,可忽略){n_negl}）", fontsize=8.5)
...
fig.text(0.5, 0.045, f"记录距离 rec_dist [m]（物理 bin=1ns=15cm，柱显示宽 {CELL_W}m；黑虚线=真实 {D_CUMUL:.0f}m；柱内=发射kick号，色=发射激光器）",
...
leg = [
    ...
    Patch(facecolor="0.8", edgecolor="0.5", linewidth=0.8, linestyle="--", label="鬼影·空间可忽略(编号间隔>%d;灰虚线半透明)" % CROSSTALK_MAX_GAP),
]
fig.legend(handles=leg, loc="upper center", ncol=3, fontsize=9.5)
plt.suptitle(f"每激光器逐回波堆叠柱（物体 D={D_CUMUL:.0f}m）—— 每个回波一个柱，柱内写发射kick，颜色=发射激光器",
             fontsize=13, y=0.975)
plt.tight_layout(rect=[0.03, 0.06, 0.98, 0.93])
plt.savefig("crosstalk_v12_cumulative_tof.png", dpi=110, bbox_inches="tight")
plt.show()
print("已保存 crosstalk_v12_cumulative_tof.png")
...
5. 输出文件
   - crosstalk_v12_timing.png          发光时序图（kick 栅格·统一配色 + kick0 时间轴放大）
   - crosstalk_v12_echo_map.png        4 个测试距离的记录距离分布（鬼影来源标注 + 可忽略串扰弱化）
   - crosstalk_v12_cumulative_tof.png  每激光器逐回波堆叠柱（柱内写kick、颜色=发射激光器）
...
cells = [
    md_cell("v12_header", CELL_HEADER),
    code_cell("v12_imports", CELL_IMPORTS),
    code_cell("v12_excel", CELL_EXCEL),
    code_cell("v12_encoding", CELL_ENCODING),
    code_cell("v12_timing_plot", CELL_TIMING_PLOT),
    code_cell("v12_detect", CELL_DETECT),
    code_cell("v12_verify", CELL_VERIFY),
    code_cell("v12_echo_map", CELL_ECHO_MAP),
    code_cell("v12_cumulative", CELL_CUMULATIVE),
    code_cell("v12_summary", CELL_SUMMARY),
]
