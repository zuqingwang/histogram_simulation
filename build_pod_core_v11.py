# -*- coding: utf-8 -*-
"""从 PoD_esti_v11.ipynb 提取计算内核 → pod_esti_v11_core.py（供多进程 import）。
...
NB = Path("PoD_esti_v11.ipynb")
OUT = Path("pod_esti_v11_core.py")
CALC_CELLS = [2, 4, 6, 8, 9, 17, 22, 25]
CELL17_CUT = "# ---- ★ v11：统一 BG_GRID"
...
def main():
    ...
    for idx in CALC_CELLS:
        ...
        if idx == 22:
            ...
            if ti >= 0:
                src = src[:ti].rstrip() + "\n"
        parts.append(f"\n# ===== 源自 PoD_esti_v11.ipynb cell {idx} =====\n")
        parts.append(src)
    parts.append(FOOTER)
